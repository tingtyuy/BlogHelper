...
const logger = require("logger2x").createLogger(
  `${require("os").homedir()}/BlogHelper/publish.log`
);

// 上传文章
function publishArticleTo(tray, sites, isPublish, sleep) {
  // 1.选择本地文件
  ...
  for (const site of sites) {
    ...
    let handler = function () {
      if (i < files.length) {
        ...
        i++;
        appPublish
          .publishArticleTo(title, content, dirname, site, isPublish)
          .then((url) => {
  ...
  for (let i = 0; i < result.files.length; i++) {
    ...
    for (let [src, fullPath] of map.entries()) {
      if (
        ...
        appUtil.isLocalPicture(fullPath)
      ) {
        await appUpload
          .uploadPicture(fullPath)
          .then((href) => {
  ...
  await appUpload
    ...
    .then((href) => {
      let number = dialog.showMessageBoxSync({
        message: "图片链接已获取，拷贝格式为",
        buttons: ["Markdown", "HTML", "URL"],
      });
      if (number === 0) {
        ...
        while (!clipboard.readImage().isEmpty()) {
          clipboard.writeText(
            `<img src="${href}" referrerpolicy="no-referrer"/>`
          );
        }
      } else if (number === 2) {
        while (!clipboard.readImage().isEmpty()) {
