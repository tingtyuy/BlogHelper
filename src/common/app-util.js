...
exports.myGetMenuItemByLabelPath = function myGetMenuItemByLabelPath(
  path,
  myMenu
) {
  let fount;
 ...
 * 取指定字数的文章摘要
 */
exports.getArticeBrief = function getArticeBrief(article, wordCount) {
  const summary = article.replace(/<[^>]*>?/gm, ''); 
  let articeBrief = summary.substring(0, wordCount);
  if (articeBrief.length < wordCount) {
    let count = wordCount - articeBrief.length;
